...
void Shader::initGeoShader(const std::string& vertFile, const std::string& geomFile, const std::string& fragFile)
{
	program = glCreateProgram(); // create shader program (openGL saves as ref number)
	shaderGeo[0] = CreateShader(LoadShader(vertFile), GL_VERTEX_SHADER); // create vertex shader
	shaderGeo[1] = CreateShader(LoadShader(geomFile), GL_GEOMETRY_SHADER); // create vertex shader
	shaderGeo[2] = CreateShader(LoadShader(fragFile), GL_FRAGMENT_SHADER); // create fragment shader

	for (unsigned int i = 0; i < Geo_Shader_Total; i++)
	{
		glAttachShader(program, shaderGeo[i]); //add all our shaders to the shader program "shaders" 
	}
